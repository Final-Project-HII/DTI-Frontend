...
const ProductSelect: React.FC<ProductSelectProps> = ({
    ...
    const filteredProductsNotInWarehouse = filteredProducts.filter((product) => {
        const productStocks = product.stocks;
        return !productStocks.some((stock) => stock.warehouseId.toString() === selectedWarehouseId);
    });
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
            ...
            <PopoverContent className="p-0 w-[25vw] z-40 border">
                <Command>
                    ...
                    />
                    <CommandList>
                        {filteredProductsNotInWarehouse.length === 0 && (
                            <CommandEmpty>No matching products found.</CommandEmpty>
                        )}
                        <CommandGroup>
                            {filteredProductsNotInWarehouse.map((product) => (
                                <CommandItem
                                    key={product.id}
                                    onSelect={() => {
